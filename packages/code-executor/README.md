...
### Если сборка зависает на загрузке слоёв Docker

При `docker compose up -d` загрузка слоя образа (например `golang:1.24-alpine`) может зависнуть на 0B из‑за сети или лимитов Docker Hub. Что сделать:

1. **Повторить сборку** — иногда помогает повторный запуск:
   ```bash
   docker compose build --no-cache code-executor
   docker compose up -d
   ```
2. **Скачать базовый образ вручную** (с повторами):
   ```bash
   docker pull golang:1.24-alpine
   ```
3. **Включить BuildKit** (если ещё не включён):
   ```bash
   DOCKER_BUILDKIT=1 docker compose build code-executor
   ```
